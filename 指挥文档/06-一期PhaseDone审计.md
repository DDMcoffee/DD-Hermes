...
current_mainline_task_id: dd-hermes-independent-quality-seat-v1
current_mainline_doc: 指挥文档/08-恒定锚点策略.md
current_gap_1: phase-2 当前主线已经有第一条 quality-seat execution closeout；下一步要裁决是归档当前 task，还是继续定义独立质量位的升级边界。
current_gap_2: phase-2 当前仍是 degraded 监督形态；需要后续实现证明哪些任务必须实配独立质量位、哪些可以显式 degraded。
---
...
- 一期已经能用
- phase-2 主线已经从已归档的 `dd-hermes-anchor-governance-v1` 切换到 `dd-hermes-independent-quality-seat-v1`
- `dd-hermes-independent-quality-seat-v1` 现在已经有正式 planning task 工件、第一条 execution slice 和真实 closeout 证据
- 但它还没有 archive，也还没有裁决下一条 follow-up boundary

## 剩余 gap

- 一期无剩余 blocker。
- 剩余 gap 全部属于 phase-2：
  - 决定当前主线是在这条 first-slice closeout 上直接归档，还是继续在同一 task 内定义独立质量位升级规则
  - 决定 `independent_skeptic=false` 的 degraded 现实在哪些任务类型上必须升级为独立质量位

## 下一条唯一主线任务

下一条唯一主线已经定义为 `dd-hermes-independent-quality-seat-v1`。
...
- 当前这条主线已经落下第一条共享脚本切片，并已形成 closeout 级证据；`dispatch-create` / `thread-switch-gate` / `quality-gate` / `state-read` / `context-build` 现在都带上了 `quality seat` 真相。
...
- 不开第二个并跑主线线程。
- 下一步先裁决 `dd-hermes-independent-quality-seat-v1` 的 follow-up boundary，再决定下一轮 slice 是否需要独立 worktree。
