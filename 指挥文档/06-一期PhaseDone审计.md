...
current_gap_1: phase-2 当前主线已经进入 planning；下一步要把独立质量位的语义与首个 execution boundary 收成共享治理脚本可实现的切片。
...
- 一期已经能用
- phase-2 主线已经从已归档的 `dd-hermes-anchor-governance-v1` 切换到新的 planning task
- `dd-hermes-independent-quality-seat-v1` 现在已经有正式 planning task 工件
- 但它还没有进入第一条 implementation slice

## 剩余 gap

- 一期无剩余 blocker。
- 剩余 gap 全部属于 phase-2：
  - 把独立质量位从 planning 目标推进到共享治理脚本、gate 和测试的第一条真实实现切片
...
- 当前这条主线还是 planning task；`dispatch-create` / `thread-switch-gate` / `quality-gate` 将成为它的第一条实现落点。
...
- 不开第二个并跑主线线程。
- 下一步先完成 `dd-hermes-independent-quality-seat-v1` 的 planning 收口，再决定首个 execution slice 是否需要独立 worktree。
