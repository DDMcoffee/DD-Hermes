...
product_goal: Advance local XC-BaoXiaoAuto `web-main` to the verified M4 export-failure-state commit while preserving the user's unrelated uncommitted WIP.
user_value: The honest export failure state becomes the real local web baseline, without consuming or overwriting the user's unfinished edits.
task_class: T2
...
non_goals:
  ...
  - Do not add new product work beyond landing the verified M4 export-failure-state slice on local `web-main`.
product_acceptance:
  - Local `web-main` advances from `95bb706de73aaccd0820f0facd7a235d98408423` to `d7d19cb875f2ff89f70885fe0b1d226578e9b9b3`.
...
acceptance:
  - Local `web-main` fast-forwards to `d7d19cb875f2ff89f70885fe0b1d226578e9b9b3`.
...
The M4 export-failure-state slice already exists as a verified side branch commit. The next operational step is to make that behavior the actual local `web-main` baseline, while preserving the same six unrelated uncommitted edits that already exist on the target main worktree.
...
- In scope:
  ...
  - fast-forward local `web-main` to `d7d19cb875f2ff89f70885fe0b1d226578e9b9b3`
...
- `execution_host`: `target-repo`
- `target_repo_ref`: `d7d19cb875f2ff89f70885fe0b1d226578e9b9b3`
- Instruction surface: `workspace/handoffs/xc-baoxiao-web-m4-export-failure-state-land-v1-lead-to-executor.md`
- Evidence surface: `workspace/state/xc-baoxiao-web-m4-export-failure-state-land-v1/state.json`

## Acceptance

- Local `web-main` becomes the verified M4 baseline.
...
- Target repo side:
  - `git stash push -u`
  - `git merge --ff-only codex/xc-baoxiao-web-m4-export-failure-state-v1`
  - `npm run test`
  - `npm run typecheck`
  - `npm run build`
  - `git stash pop`
- DD Hermes side:
  - `./scripts/context-build.sh --task-id xc-baoxiao-web-m4-export-failure-state-land-v1 --agent-role commander --worktree /Volumes/Coding/XC-BaoXiaoAuto`
  - `hooks/quality-gate.sh --event Stop --state workspace/state/xc-baoxiao-web-m4-export-failure-state-land-v1/state.json`
