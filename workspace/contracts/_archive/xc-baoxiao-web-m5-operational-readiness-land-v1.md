...
product_goal: Advance local XC-BaoXiaoAuto `web-main` to the verified M5 operational-readiness commit while preserving the user's unrelated uncommitted WIP.
user_value: The M5 readiness overview becomes the real local web baseline, without consuming or overwriting the user's unfinished edits.
task_class: T2
...
non_goals:
  ...
  - Do not add new product work beyond landing the verified M5 operational-readiness slice on local `web-main`.
product_acceptance:
  - Local `web-main` advances from `de5ef1fe8b13d6111132a53867f52fcf922b445b` to `4df74ce9931f4a334b997a3145a8ad02d20e2243`.
...
acceptance:
  - Local `web-main` fast-forwards to `4df74ce9931f4a334b997a3145a8ad02d20e2243`.
...
The M5 operational-readiness slice already exists as a verified side branch commit. The next operational step is to make that administrator-facing readiness view the actual local `web-main` baseline, while preserving the same six unrelated uncommitted edits that already exist on the target main worktree.
...
- In scope:
  ...
  - fast-forward local `web-main` to `4df74ce9931f4a334b997a3145a8ad02d20e2243`
...
- `execution_host`: `target-repo`
- `target_repo_ref`: `4df74ce9931f4a334b997a3145a8ad02d20e2243`
- Instruction surface: `workspace/handoffs/xc-baoxiao-web-m5-operational-readiness-land-v1-lead-to-executor.md`
- Evidence surface: `workspace/state/xc-baoxiao-web-m5-operational-readiness-land-v1/state.json`

## Acceptance

- Local `web-main` becomes the verified M5 operational-readiness baseline.
...
- Target repo side:
  - `git merge --ff-only codex/xc-baoxiao-web-m5-operational-readiness-v1`
  - `npm run test`
  - `npm run typecheck`
  - `npm run build`
- DD Hermes side:
  - `./scripts/context-build.sh --task-id xc-baoxiao-web-m5-operational-readiness-land-v1 --agent-role commander --worktree /Volumes/Coding/XC-BaoXiaoAuto`
  - `hooks/quality-gate.sh --event Stop --state workspace/state/xc-baoxiao-web-m5-operational-readiness-land-v1/state.json`
