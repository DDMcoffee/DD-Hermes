...
product_goal: Advance local XC-BaoXiaoAuto `web-main` to the verified M4 storage-driver-contract commit while preserving the user's unrelated uncommitted WIP.
user_value: The tightened storage-driver boundary becomes the real local web baseline, without consuming or overwriting the user's unfinished edits.
task_class: T2
...
non_goals:
  ...
  - Do not add new product work beyond landing the verified M4 storage-driver-contract slice on local `web-main`.
product_acceptance:
  - Local `web-main` advances from `e68b140d5065d98cda6cf551ad404fa254541a48` to `de5ef1fe8b13d6111132a53867f52fcf922b445b`.
...
acceptance:
  - Local `web-main` fast-forwards to `de5ef1fd0d40f6ec25e65270178bd34c9d208b62`.
...
The M4 storage-driver-contract slice already exists as a verified side branch commit. The next operational step is to make that tightened storage boundary the actual local `web-main` baseline, while preserving the same six unrelated uncommitted edits that already exist on the target main worktree.
...
- In scope:
  ...
  - fast-forward local `web-main` to `de5ef1fe8b13d6111132a53867f52fcf922b445b`
...
- `execution_host`: `target-repo`
- `target_repo_ref`: `de5ef1fe8b13d6111132a53867f52fcf922b445b`
- Instruction surface: `workspace/handoffs/xc-baoxiao-web-m4-storage-driver-contract-land-v1-lead-to-executor.md`
- Evidence surface: `workspace/state/xc-baoxiao-web-m4-storage-driver-contract-land-v1/state.json`

## Acceptance

- Local `web-main` becomes the verified M4 storage-driver-contract baseline.
...
- Stop if the work expands into cleanup or new storage behavior.

## Verification

- Target repo side:
  - `git merge --ff-only codex/xc-baoxiao-web-m4-storage-driver-contract-v1`
  - `npm run test`
  - `npm run typecheck`
  - `npm run build`
- DD Hermes side:
  - `./scripts/context-build.sh --task-id xc-baoxiao-web-m4-storage-driver-contract-land-v1 --agent-role commander --worktree /Volumes/Coding/XC-BaoXiaoAuto`
  - `hooks/quality-gate.sh --event Stop --state workspace/state/xc-baoxiao-web-m4-storage-driver-contract-land-v1/state.json`
