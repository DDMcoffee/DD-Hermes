...
product_goal: Advance local XC-BaoXiaoAuto `web-main` to the verified M4 PDF-boundary-visibility commit while preserving the user's unrelated uncommitted WIP.
user_value: The explicit HTML/PDF export boundary becomes the real local web baseline, without consuming or overwriting the user's unfinished edits.
task_class: T2
...
non_goals:
  ...
  - Do not add new product work beyond landing the verified M4 PDF-boundary-visibility slice on local `web-main`.
product_acceptance:
  - Local `web-main` advances from `2a6293508e9efd521073b5604f02f3660315e2b1` to `e68b140d5065d98cda6cf551ad404fa254541a48`.
...
acceptance:
  - Local `web-main` fast-forwards to `e68b140d5065d98cda6cf551ad404fa254541a48`.
...
The M4 PDF-boundary-visibility slice already exists as a verified side branch commit. The next operational step is to make that explicit HTML/PDF export boundary the actual local `web-main` baseline, while preserving the same six unrelated uncommitted edits that already exist on the target main worktree.
...
- In scope:
  ...
  - fast-forward local `web-main` to `e68b140d5065d98cda6cf551ad404fa254541a48`
...
- `execution_host`: `target-repo`
- `target_repo_ref`: `e68b140d5065d98cda6cf551ad404fa254541a48`
- Instruction surface: `workspace/handoffs/xc-baoxiao-web-m4-pdf-boundary-visibility-land-v1-lead-to-executor.md`
- Evidence surface: `workspace/state/xc-baoxiao-web-m4-pdf-boundary-visibility-land-v1/state.json`

## Acceptance

- Local `web-main` becomes the verified M4 PDF-boundary-visibility baseline.
...
- Stop if the work expands into cleanup or new export behavior.

## Verification

- Target repo side:
  - `git stash push -u`
  - `git merge --ff-only codex/xc-baoxiao-web-m4-pdf-boundary-visibility-v1`
  - `npm run test`
  - `npm run typecheck`
  - `npm run build`
  - `git stash pop`
- DD Hermes side:
  - `./scripts/context-build.sh --task-id xc-baoxiao-web-m4-pdf-boundary-visibility-land-v1 --agent-role commander --worktree /Volumes/Coding/XC-BaoXiaoAuto`
  - `hooks/quality-gate.sh --event Stop --state workspace/state/xc-baoxiao-web-m4-pdf-boundary-visibility-land-v1/state.json`
