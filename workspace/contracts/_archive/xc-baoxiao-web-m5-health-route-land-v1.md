...
product_goal: Advance local XC-BaoXiaoAuto `web-main` to the verified M5 health-route commit while preserving the user's unrelated uncommitted WIP.
user_value: The machine-readable M5 readiness snapshot becomes the real local web baseline, without consuming or overwriting the user's unfinished edits.
task_class: T2
...
non_goals:
  ...
  - Do not add new product work beyond landing the verified M5 health-route slice on local `web-main`.
product_acceptance:
  - Local `web-main` advances from `4df74ce9931f4a334b997a3145a8ad02d20e2243` to `43446505f03a382fe57f8ea837cf744019f0ca27`.
...
acceptance:
  - Local `web-main` fast-forwards to `43446505f03a382fe57f8ea837cf744019f0ca27`.
...
The M5 health-route slice already exists as a verified side branch commit. The next operational step is to make that machine-readable readiness surface the actual local `web-main` baseline, while preserving the same six unrelated uncommitted edits that already exist on the target main worktree.
...
- In scope:
  ...
  - fast-forward local `web-main` to `43446505f03a382fe57f8ea837cf744019f0ca27`
...
- `execution_host`: `target-repo`
- `target_repo_ref`: `43446505f03a382fe57f8ea837cf744019f0ca27`
- Instruction surface: `workspace/handoffs/xc-baoxiao-web-m5-health-route-land-v1-lead-to-executor.md`
- Evidence surface: `workspace/state/xc-baoxiao-web-m5-health-route-land-v1/state.json`

## Acceptance

- Local `web-main` becomes the verified M5 health-route baseline.
...
- Target repo side:
  - `git merge --ff-only codex/xc-baoxiao-web-m5-health-route-v1`
  - `npm run test`
  - `npm run typecheck`
  - `npm run build`
- DD Hermes side:
  - `./scripts/context-build.sh --task-id xc-baoxiao-web-m5-health-route-land-v1 --agent-role commander --worktree /Volumes/Coding/XC-BaoXiaoAuto`
  - `hooks/quality-gate.sh --event Stop --state workspace/state/xc-baoxiao-web-m5-health-route-land-v1/state.json`
