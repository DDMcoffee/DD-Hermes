...
product_goal: Advance local XC-BaoXiaoAuto `web-main` to the verified M5 storage-probe-readiness commit while preserving the user's unrelated uncommitted WIP.
user_value: Local `web-main` gains honest storage-readiness signaling for M5 without consuming or overwriting the user's unfinished edits.
task_class: T2
quality_requirement: degraded-allowed
task_class_rationale: This is a bounded cross-repo landing step with branch movement, post-landing verification, and WIP preservation. It is operationally sensitive enough for S2 traceability but does not require architecture-scale exploration.
non_goals:
  - Do not push remote branches or tags.
  - Do not modify the substance of the user's existing six-path WIP.
  - Do not add new product work beyond landing the verified M5 storage-probe-readiness slice on local `web-main`.
product_acceptance:
  - Local `web-main` advances from `d899931e66861530533383cfe7ea40df662b312d` to `ec44bf504be0d3fdd45518ba8a9332b99e968a68`.
...
acceptance:
  - Local `web-main` fast-forwards to `ec44bf504be0d3fdd45518ba8a9332b99e968a68`.
...
The M5 storage-probe-readiness slice already exists as a verified side branch commit. The next operational step is to make that honest storage-readiness behavior the actual local `web-main` baseline, while preserving the same six unrelated uncommitted edits that already exist on the target main worktree.
...
- In scope:
  ...
  - fast-forward local `web-main` to `ec44bf504be0d3fdd45518ba8a9332b99e968a68`
...
- `execution_host`: `target-repo`
- `target_repo_ref`: `ec44bf504be0d3fdd45518ba8a9332b99e968a68`
- Instruction surface: `workspace/handoffs/xc-baoxiao-web-m5-storage-probe-readiness-land-v1-lead-to-executor.md`
- Evidence surface: `workspace/state/xc-baoxiao-web-m5-storage-probe-readiness-land-v1/state.json`

## Acceptance

- Local `web-main` becomes the verified M5 storage-probe-readiness baseline.
...
- Target repo side:
  - `git merge --ff-only codex/xc-baoxiao-web-m5-storage-probe-readiness-v1`
  - `npm run test`
  - `npm run typecheck`
  - `npm run build`
- DD Hermes side:
  - `./scripts/context-build.sh --task-id xc-baoxiao-web-m5-storage-probe-readiness-land-v1 --agent-role commander --worktree /Volumes/Coding/XC-BaoXiaoAuto`
  - `hooks/quality-gate.sh --event Stop --state workspace/state/xc-baoxiao-web-m5-storage-probe-readiness-land-v1/state.json`
